...
class IntelligenceService {
  IntelligenceModel analyzeInventory({
    ...
    if (expired > 0 || critical > 0 || urgentRenewals > 0) {
      riskLevel = 'High';
    ...
    if (expired > 0) {
      recommendations.add(
        'Immediate action required: $expired certificate(s) have expired.',
      );
    }

    if (critical > 0) {
      recommendations.add(
        '$critical certificate(s) are critical and should be renewed within 30 days.',
      );
    }

    if (warning > 0) {
      recommendations.add(
        '$warning certificate(s) should be reviewed before expiry.',
      );
    }

    if (possibleDsc == 0) {
      recommendations.add(
        'No possible DSC certificates detected. Verify certificate availability.',
      );
    }

    if (usbTokens.isEmpty) {
      recommendations.add(
        'No signing/token devices detected. Verify whether signing devices are required.',
      );
    }
